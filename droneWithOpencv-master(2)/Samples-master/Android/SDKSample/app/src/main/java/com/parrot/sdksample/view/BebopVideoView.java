                    ...
                    if (b != null) {
                        ...
                        byte[] data = frame.getByteData();
                            ...
                            //synchronized (this) {
                                ...
                                if (data.length == 0)
                                    Log.i("frame: ", "frame is not read");
                                else {
                                    Log.i("frame: ", "frame is read");
                                    Log.i("frame length: ", "frame length is: " + data.length);
                                }
                                mBmp = BitmapFactory.decodeByteArray(data, 0, data.length);
                               // ByteArrayInputStream ins = new ByteArrayInputStream(data);
                               // mBmp = BitmapFactory.decodeStream(ins);
                        // }

                            String path = Environment.getExternalStorageDirectory().getPath() + "/mfile.jpg";
                            ...
                            try {
                                ...
                                if (mBmp != null) {
                                    ...
                                    out.write(mBmp.toString().getBytes());
                                }
                                else{
